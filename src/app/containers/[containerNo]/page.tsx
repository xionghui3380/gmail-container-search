...
export default function ContainerDetailPage({
  ...
  const loadItems = useCallback(async () => {
    ...
    const res = await fetch(
      `/api/v1/containers/by-no/${encodeURIComponent(containerNo)}/items?${params}`,
    );
    const json = await res.json();
    if (!res.ok) {
      toast.error(json.message ?? "明细加载失败");
      return;
    }
    setItems(json.data ?? []);
    ...
    try {
      ...
      if (!res.ok) {
        if (json.meta?.needReconnect) {
          toast.error("Gmail 未连接或已过期，请先在列表页连接 Gmail");
        } else {
      ...
      if (json.data.warnings?.length) {
        toast.warning(json.data.warnings.join("；"));
      }
      await loadAll();
    } finally {
      setParsing(false);
        ...
        <div className="space-y-4">
          <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-3 flex flex-wrap items-center gap-3">
              <h2 className="text-base font-semibold text-slate-800">基础信息</h2>
              <span
                className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                  PARSE_STATUS_CLASS[detail.parse_status] ?? PARSE_STATUS_CLASS.pending
                }`}
              >
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <InfoItem label="柜型" value={detail.container_type} />
              <InfoItem label="码头" value={detail.terminal} />
              <InfoItem label="客户" value={detail.customer} />
              <InfoItem label="MBL" value={detail.mbl || "-"} />
              <InfoItem
                label="操作方式"
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-slate-50 text-left text-slate-600">
                  <tr>
                    ...
                    <th className="px-4 py-2 font-medium">明细条数</th>
                  </tr>
                </thead>
                <tbody>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder="搜索仓库/FBA/Reference"
                  className="h-8 w-48 rounded border border-slate-200 px-2 text-sm outline-none focus:border-blue-400"
                />
                <button
                  onClick={() => loadItems()}
